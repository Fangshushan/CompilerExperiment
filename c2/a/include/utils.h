    ...
    #define ALLOCATE_EXTRA(vmPtr,mainType,extrSize) \
     (mainType *)memManager(vmPtr,NULL,0,sizeof(mainType)+extrSize)
    ...
    #define DEFINE_BUFFER_METHOD(type) \
        ...
        void type##BufferClear(VM* vm,type##Buffer *buf){ \
            size_t  oldSize = buf->capacity * sizeof(buf->data[0]); \
            memManager(vm,buf->datas,oldSize,0);\
            type##BufferInit(buf);\
        }

    DECLARE_BUFFER_TYPE(String)
    #define SymbolTable StringBuffer; // 一声明就报错 
    ...
    void symbolTableClear(VM*,StringBuffer* buffer);
